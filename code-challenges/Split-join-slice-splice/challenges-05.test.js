...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach((el) => {
    let template = $('#template').clone().removeAttr('id');
  
    // template.removeAttr('id');

    template.find('h2').text(el.name);
    template.find('h3').text(el.height);
    template.find('p').text(el.eye_color);
...
//   }
  for (let i = 0; i < str.length + 1 ; i++) {
    let resStr = str.slice(i, str.length)
    result.push(resStr);
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(el => {
    result.push(el.slice(el.indexOf(' ', 3)+1))
  })
