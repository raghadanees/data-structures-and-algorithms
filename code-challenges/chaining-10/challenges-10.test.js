...
const createServer = () => {
  ...
  app.get('/aboutme',(request,response)=>{
    response.status(200).send('I am Raghad ');
  });

  app.get('/favoritefoods',(request,response)=>{
    response.status(200).send(['Yallengi','Kobba']);
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let resultArr = input.map((el) => {
    return el.filter((el) => {
      return el % 5 === 0 && typeof el === 'number';
    });
  });

  return resultArr.map((element) => {
    return element.map((element) => {
