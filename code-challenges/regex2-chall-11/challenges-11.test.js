...
//       return false
//   }
let patternOfPin = /^\d{4}$/g;
  return patternOfPin.test(pin);
};
...
const validateEmail = (email) => {
  // Solution code here...
  let PatternOfEmail = /^[A-Za-z0-9]+\.?[A-Za-z0-9]+?@[A-Za-z0-9]+.(net|com|org)$/;
  return PatternOfEmail.test(email);
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let phoneNumPattern = /^(\(\d{3}\)|\d{3})[- ]?\d{3}[- ]?\d{4}$/;
  return phoneNumPattern.test(phoneNumber);
};
